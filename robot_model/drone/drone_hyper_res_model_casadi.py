...
def hyper_residual_drone_dynamics(const_params=None, layer_sizes=[10, 32, 6], activation=cs.tanh):
    ...
    f_dist_hyper = cs.MX.sym("f_dist_hyper", 3)
    
    f_dist = cs.vertcat(f_x, f_y, f_z) + f_dist_hyper
    ...
    all_params = cs.vertcat(p_6, p_mlp, f_dist_hyper)
    ...
    f_dist_hyper_val = np.zeros(3)
    default_params_val = np.concatenate((default_nominal_params, default_mlp_params, f_dist_hyper_val))
