...
def car_dynamics(p_init, track_path : str, cfg, cfg_model=None, errors=False, sens_error=False):
    ...
    [s0, _, _, _, kapparef, track_width] = getTrackCustom(track_path, return_width=True)    
    ...
    width_s = cs.interpolant("width_s", "bspline", [s0], track_width)
    ...
    h_left = width_s(s) / 2 - n
    h_right = width_s(s) / 2 + n
    ...
    strat_drift_cost_s = track_length - 2
    enable_drift_cost = (cs.tanh(s - strat_drift_cost_s)+1) / 2
    drift_cost =   100 * (mu - 3.14 / 2 )**2
     
    
    cost_expr = 20.0 * (s - track_length)**2 +  cfg.q_track_constraint * track_soft_constraints_cost + drift_cost 
           
    cost_expr_e = 1000.0 * (s - track_length)**2 + 200 * (mu - 3.14 / 2 )**2 + cfg.q_track_constraint * track_soft_constraints_cost \
        + 10 * vx**2 +  10 * vy**2 
    ...
    p_global = None
    if sens_error:
        print("Using sensitivity errors in the model")
        # selection matrix of the errors
        # S(n) = [9, N+1]
        S_n = cs.MX.sym('S', 1, 81) # FIXME the N size is hardcoded here, should be dynamic\
        e_global = cs.MX.sym('e_global', 81, 9) # 9 * (N+1) = 81
        
        p_global = cs.vertcat(cs.reshape(e_global, -1, 1)) 
        
        e_n = cs.mtimes(S_n, e_global).T
        x_next = x_next + e_n
        
        # add to parameters
        p_dyn_model = cs.vertcat(p_dyn_model, cs.reshape(S_n, -1, 1))
        p_init = np.concatenate([p_init, np.zeros(S_n.shape[1])])
        print(f"p_init shape: {p_init.shape}, p_global shape: {p_global.shape}")
