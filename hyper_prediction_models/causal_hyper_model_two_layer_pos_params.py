...
class CausalHyperModel2p(torch.nn.Module):
    def __init__(self,
        ...
        super(CausalHyperModel2p, self).__init__()
        ...
        # assetr that always_positive is a tensor of bools
        assert torch.is_tensor(always_positive)
        
        self.always_positive_idx = always_positive
        ...
        default_params_pos = default_params[..., self.always_positive_idx]
        assert torch.all(default_params_pos > 0.0)
        default_params = default_params.clone()
        default_params[..., self.always_positive_idx] = torch.log(default_params_pos)
    ...
    def forward_without_tse(self, hn, P, enable_prediction_head):
        ...
        free_params = fc_out[..., self.free_params].clone()

        p_delta = 1.0 + torch.nn.Tanh()(fc_out) * self.max_param_scaler
        
        if self.last_ctrl_point_on_const:
            p_last = torch.ones(hn.shape[0], self.param_size, device=self.device)
            p_delta[:, -1, :] = p_last            
        
        p_const_exp = self.const_params.clone()
        p_const_exp[..., self.always_positive_idx] = self.const_params[..., self.always_positive_idx].exp().clone()

        p_ret = p_delta * p_const_exp
        
        p_ret[..., self.free_params] = free_params
