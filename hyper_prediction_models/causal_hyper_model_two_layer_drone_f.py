...
class CausalHyperModel2f(torch.nn.Module):
    def __init__(self,
        ...
        super(CausalHyperModel2f, self).__init__()
    ...
    def forward_without_tse(self, hn, P, enable_prediction_head):
        ...
        fc_out = torch.nn.Tanh()(fc_out) * self.max_param_scaler

        p_delta = fc_out.view(-1, self.param_ctrl_point, self.param_size)

        if self.last_ctrl_point_on_const:
            p_last = torch.zeros(hn.shape[0], self.param_size, device=self.device)
            # FIXME p_delta = torch.cat((p_delta, p_last), dim=1)
            p_delta[:, -1, :] = p_last

        p_ret = p_delta

        return p_ret, p_delta + 1.0
