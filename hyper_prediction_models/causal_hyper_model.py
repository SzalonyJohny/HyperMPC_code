...
class CausalHyperModel(torch.nn.Module):
    def __init__(self,
                 ...
                 hl2_size: int, # 32
                 predictions_botteckleneck_size: int, # 3
                 ...
                 disable_pred_channel_mixer: bool = False,
                 ):

        super(CausalHyperModel, self).__init__()
        ...
        #                                        self.predictions_botteckleneck_size,
        #                                        self.m1_feature_size,
        #                                        bias=False).to(self.device)
        # self.hl1_to_m1 = torch.nn.Linear(hl1_size, m1_feature_size * self.param_ctrl_point,
        ...
        self.causal_mlp_2 = CausalMaskedLinear(self.param_ctrl_point,
                                               ...
                                               bias=False).to(self.device)
        self.hl2_to_params = torch.nn.Linear(hl2_size, self.param_size * self.param_ctrl_point, 
                                             bias=True).to(self.device)
        ...
        torch.nn.init.normal_(self.predictions_channels_mixer.weight, mean=0.0, std=0.001)
        self.predictions_channels_mixer.bias.data.fill_(0)
    ...
    def forward_without_tse(self, hn, P, enable_prediction_head):
        ...
        s = spline_ctrl_points.view(-1, self.future_predictions_channel_count)
        l = self.predictions_channels_mixer(s).view(-1, self.param_ctrl_point *
                                                   self.predictions_botteckleneck_size)
        ...
        # Causal second layer
        fc_out = self.causal_mlp_2(l) + self.hl2_to_params(hl2_act)
