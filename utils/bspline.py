...
class BSpline:
    def __init__(self, n, d=3, num_T_pts=1024, name=""):
        ...
        self.u = np.pad(np.linspace(0., 1., self.m +
                        1 - 2 * self.d), self.d, 'edge')
        ...
        if os.path.exists(fname):
            ...
            np.save(fname, {"N": self.N, "dN": self.dN,
                    "ddN": self.ddN, "dddN": self.dddN}, allow_pickle=True)
        ...
        self.N_ = torch.Tensor(self.N)
        self.dN_ = torch.Tensor(self.dN)
        self.ddN_ = torch.Tensor(self.ddN)
        self.dddN_ = torch.Tensor(self.dddN)
    ...
    def calculate_N(self):
        def N(n, t, i):
            ...
            if self.u[i + n] - self.u[i] != 0:
                s += (t - self.u[i]) / \
                    (self.u[i + n] - self.u[i]) * N(n - 1, t, i)
            if self.u[i + n + 1] - self.u[i + 1] != 0:
                s += (self.u[i + n + 1] - t) / (self.u[i + n +
                                                       1] - self.u[i + 1]) * N(n - 1, t, i + 1)
            return s
